...
import { normalizeCertNo } from './inventory.service'
...
export async function importFromExcel(buffer: Buffer): Promise<{ imported: number; updated: number; errors: string[] }> {
  const wb = new ExcelJS.Workbook()
  await wb.xlsx.load(buffer as unknown as ExcelJS.Buffer)
  const ws = wb.worksheets[0]
  if (!ws) return { imported: 0, updated: 0, errors: ['Excel 无工作表'] }
  ...
  ws.eachRow((row, rowNumber) => {
    ...
    tasks.push(
      prisma.bracelet
        ...
        .then((r) => {
          if (r.createdAt.getTime() === r.updatedAt.getTime()) imported++
          else updated++
        })
        .catch((e) => {
          errors.push(`${certNo}: ${e instanceof Error ? e.message : String(e)}`)
        })
        .then(() => undefined),
    )
  })
